...
function getOppositeSide(side: TDropdownPosition): TDropdownPosition {
  switch (side) {
    case "top":
      return "bottom";
    case "bottom":
...
function getOppositeAlign(align: TDropdownAlign): TDropdownAlign {
  switch (align) {
    case "start":
      return "end";
    case "end":
...
/**
 * Builds an ordered list of placements to try, starting with the preferred
 * side/align and falling back to the opposite side, recentred alignments, and
 * finally the perpendicular sides.
 */
function getPlacementCandidates(
  ...
  const pushUnique = (candidate: Placement): void => {
    if (
      !placements.some(
        (placement) =>
...
/**
 * Resolves the best side/alignment placement for floating content. When
 * `avoidCollisions` is enabled it scores every candidate placement against the
 * available viewport space and returns the one with the least overflow and
 * displacement from the preferred placement.
 */
...
/**
 * Returns a `transform-origin` value that matches the resolved placement so the
 * open/close scale animation grows from the edge nearest the trigger.
 */
